...
HTNTask::HTNTask(const EHTNTaskType inType, std::unique_ptr<const HTNValue> inName, const std::shared_ptr<const HTNMethod>& inMethod)
	: mType(inType), mName(std::move(inName)), mMethod(inMethod)
{
}

HTNTask::HTNTask(const EHTNTaskType inType, std::unique_ptr<const HTNValue> inName, const std::shared_ptr<const HTNMethod>& inMethod, const std::vector<std::shared_ptr<const HTNValue>>& inArguments)
